    ...
    <div class="card shadow-sm border-0">
        ...
        <div class="card-body">
            ...
            <table class="table table-striped table-bordered">
                ...
                <tbody>
                    @foreach($pembayaran->transaksi->komponens as $index => $komponen)
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $komponen->nama_komponen }}</td>
                        <td>Rp {{ number_format($komponen->harga, 0, ',', '.') }}</td>
                        <td>{{ $komponen->pivot->jumlah }}</td>
                        <td>Rp {{ number_format($komponen->pivot->sub_total, 0, ',', '.') }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
            ...
            <table class="table table-bordered">
                <tr>
                    ...
                    <td>{{ $pembayaran->transaksi->supplier->nama_supplier }}</td>
                </tr>
                <tr>
                    <th>Alamat</th>
                    <td>{{ $pembayaran->transaksi->supplier->alamat }}</td>
                </tr>
            </table>
