...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchInput.addEventListener('input', function() {
            ...
            fetch(`/admin/transaksi/search?query=${query}`)
                .then(res => res.json())
                .then(data => {
                    ...
                    if (data.length > 0) {
                        data.forEach(item => {
                            ...
                            li.textContent = `${item.kode_transaksi} - ${item.supplier.nama_supplier} (Rp${parseInt(item.total_harga).toLocaleString('id-ID')})`;
                            li.addEventListener('click', () => {
                                idTransaksiInput.value = item.id;
                                document.getElementById('kode_transaksi').value = item.kode_transaksi;
                                document.getElementById('nama_supplier').value = item.supplier.nama_supplier;
